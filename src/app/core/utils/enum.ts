...
export function isEnumValue<T extends Enum>(enumSrc: T, value: unknown): value is T[keyof T] {
    return Number.isInteger(enumSrc[enumSrc[value as keyof T] as unknown as keyof T]);
}

export function enumToKeys<T extends Enum>(enumSrc: T): (keyof T)[] {
    return Object.keys(enumSrc).filter((key: keyof T | unknown) => isEnumKey(enumSrc, key)) as (keyof T)[];
}
