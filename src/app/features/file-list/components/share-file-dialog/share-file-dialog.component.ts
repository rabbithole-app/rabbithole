...
export class ShareFileDialogComponent {
    ...
    readonly data = inject<{ item: FileInfoExtended }>(MAT_DIALOG_DATA);
    readonly #dialogRef = inject<MatDialogRef<ShareFileDialogComponent>>(MatDialogRef);
    shareControl = new FormControl(false);
    sharedWithControl = new FormControl('users');
    usersControl = new FormControl(this.users, [Validators.required]);
    ...
    // publicLinkControl = new FormControl('test');
    // #clipboard = inject(Clipboard);

    // timelockControl = new FormControl(false);
    // #locale: Locale = inject(MAT_DATE_LOCALE);
    // #adapter = inject(DateAdapter);
    // #translocoService = inject(TranslocoService);
    // minDate = startOfTomorrow();
    // dateControl = new FormControl(add(this.minDate, { days: 1 }), [Validators.required]);
    ...
    constructor() {
        ...
        this.shareControl.valueChanges
            .pipe(
                startWith(this.shareControl.value),
                switchMap(enabled => iif(() => enabled ?? false, this.sharedWithControl.valueChanges.pipe(startWith(this.sharedWithControl.value)), EMPTY)),
                takeUntilDestroyed()
            )
            .subscribe();*/
    }
    ...
    copy(event: MouseEvent) {
        ...
        this.#clipboard.copy(this.publicLinkControl.value ?? '');
    }*/

    handleShare(event: MouseEvent) {
        const data = {
            sharedWith: { users: this.usersControl.value?.map(v => Principal.fromText(v)) }
        };
